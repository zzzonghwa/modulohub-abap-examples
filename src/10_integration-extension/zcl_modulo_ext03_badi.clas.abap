...
"! <p>BAdI·Enhancement — released 확장점 소비(개념).</p>
"! <p>실 BAdI 정의·구현·활성화는 시스템 의존(SE18/SE19·Enhancement Spot)이라</p>
"! <p>abapGit 예제로 만들 수 없다. 따라서 소비자 관점의 *런타임 시맨틱*만 인메모리</p>
"! <p>BAdI 프레임워크(locals_imp)로 자체완결 시연한다 — GET BADI/CALL BADI 동작을 정확히 흉내.</p>
"! <p>시연 범위:</p>
"! <ul>
"! <li>CALL BADI 멀티캐스트: multiple-use BAdI는 active 구현을 전부 실행.</li>
"! <li>active/inactive override: inactive 구현은 실행에서 제외.</li>
"! <li>single-use 예외: 구현 0개 -> NOT_IMPLEMENTED, 복수 -> MULTIPLY.</li>
"! <li>fallback class: single-use 구현 0개일 때 fallback으로 폴백해 예외 회피.</li>
"! <li>인스턴스 모드: instance reuse면 동일 plug-in이 상태를 누적.</li>
"! <li>backward compatibility: 미구현 메서드는 빈 구현으로 처리(no-op).</li>
"! <li>filter 라우팅: filter 값으로 구현 선택, filter name은 대문자 정규화.</li>
"! </ul>
CLASS zcl_modulo_ext03_badi DEFINITION
  ...
  PUBLIC SECTION.
    ...
    "! CALL BADI 멀티캐스트 — multiple-use BAdI에 두 active 구현(음수금지·짝수만)을
    "! 등록하고 모두 실행해 위반 건수를 센다. 단일 전략과 달리 전부 호출된다.
    ...
    "! active/inactive override — 짝수 구현을 inactive로 등록하면 Switch/filter와
    ...
    "! multiple-use no-op — 구현 0개여도 예외 없이 위반 0(single-use와의 차이).
    "! @parameter value  | 검사할 값
    ...
    "! single-use 구현 0개 + fallback 없음 -> CX_BADI_NOT_IMPLEMENTED 비유.
    "! @parameter result | 예외가 발생하면 abap_true
    METHODS single_use_no_impl_raises
      RETURNING VALUE(result) TYPE abap_bool.

    "! single-use 구현 0개 + fallback 있음 -> 폴백되어 예외 없음.
    ...
    "! single-use 구현 복수 -> CX_BADI_MULTIPLY_IMPLEMENTED 비유.
    "! @parameter result | 예외가 발생하면 abap_true
    METHODS single_use_multiply_raises
      RETURNING VALUE(result) TYPE abap_bool.

    "! 인스턴스 모드 — instance reuse면 동일 plug-in이 재사용돼 호출 횟수가 누적된다.
    "! 같은 BAdI object에서 2회 호출 후 plug-in의 누적 호출 수를 돌려준다.
    "! @parameter result | 동일 인스턴스의 누적 check 호출 수(=2)
    METHODS instance_reuse_calls
      RETURNING VALUE(result) TYPE i.

    "! filter 라우팅 — filter 값으로 구현을 선택한다. filter name 대조용으로
    "! 소문자 입력도 대문자로 정규화해 매칭된다(대문자 필수).
    ...
    "! filter 미매칭 -> 매칭 구현 없음(NOT_IMPLEMENTED 비유).
...
CLASS zcl_modulo_ext03_badi IMPLEMENTATION.
  METHOD if_oo_adt_classrun~main.
    ...
    out->write( `[멀티캐스트] multiple-use BAdI는 active 구현을 모두 실행한다` ).
    ...
    out->write( `[active/inactive] inactive 구현은 Switch/filter 무관하게 제외된다` ).
    ...
    out->write( `[single vs multiple] 구현 0개의 차이` ).
    ...
    out->write( `[인스턴스 모드] instance reuse면 동일 plug-in이 상태를 누적한다` ).
    out->write( |instance_reuse_calls      = { instance_reuse_calls( ) } (2회 재사용 호출)| ).
    out->write( `[filter 라우팅] filter 값으로 구현 선택, filter name 대문자 정규화` ).
  ...
  METHOD validate_with_inactive.
    " 짝수 구현은 inactive로 등록 -> 실행에서 제외(active/inactive override).
    DATA(badi) = NEW lcl_multi_use_badi( ).
    badi->add_implementation( NEW lcl_non_negative( ) ).
  ...
  METHOD validate_empty_multi.
    " multiple-use BAdI에 구현 0개 -> 예외 없이 no-op.
  ...
  METHOD single_use_fallback.
    " single-use BAdI, 구현 0개지만 fallback 등록 -> 폴백되어 예외 없음.
  ...
  METHOD unknown_filter_raises.
    " 등록되지 않은 filter 값 -> 매칭 구현 없음(폴백 실패).
    DATA(badi) = NEW lcl_filter_badi( ).
    badi->register( filter_value   = `EVEN`
                    implementation = NEW lcl_even_only( ) ).
    TRY.
