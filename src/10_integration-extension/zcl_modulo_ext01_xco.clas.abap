...
"! <p>Released API·XCO(eXtension Components) 소비 — 고전 FM의 released 대체(노트 10-01).</p>
...
"! <p>소절 대응(노트 본문):</p>
"! <ul>
"! <li>문자열 체이닝(claim 18): xco_cp=>string( )->to_upper/lower/from/to/append/
"! prepend/split, xco_cp=>strings( )->join, ->starts_with/ends_with/matches.</li>
"! <li>UUID(claim 17): xco_cp=>uuid( )->value / ->as( c36 ), cl_system_uuid 대안.</li>
"! <li>현재 순간(claim 16): xco_cp=>sy->date/user — sy-datum 직접 접근의 Cloud 호환 경로.</li>
"! <li>랜덤(claim 19): cl_abap_random_int — 비released 고전 랜덤 FM 대체(C1).</li>
