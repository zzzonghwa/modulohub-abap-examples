...
"! authorization buffer(W-11)의 비유. 실 시스템은 DB를 직접 읽지 않고 이 버퍼만 검사한다.
"! 여기선 AUTHORITY-CHECK 통과 논리(W-01b: OR across authorizations · AND across fields),
"! DUMMY(W-05), sy-subrc 체계(W-03: 0/4/12), FOR USER(W-07)를 로컬 모델로 자체완결 시연한다.
"! 실 코드는 lcl_real_authority가 진짜 AUTHORITY-CHECK 구문을 감싼다(L-01 의존성 래핑).

"! 권한 체크 dependency 추상화(L-01). 단위 테스트에서 test double로 교체 가능.
INTERFACE lif_authority.
  TYPES:
    "! 체크할 field-value 쌍. value가 비어 있으면 DUMMY로 간주(W-05).
  ...
  "! AUTHORITY-CHECK 한 번에 대응. sy-subrc 호환 코드를 돌려준다(W-03).
  "! @parameter object | authorization object 이름(대문자, W-02)
  "! @parameter fields | 체크할 field-value 쌍 목록(최대 10개, W-04)
  "! @parameter user   | FOR USER 대상(비우면 현재 사용자, W-07)
...
"! sy-subrc 호환 상수(W-03). 의미를 코드에 명시한다.
INTERFACE lif_subrc.
  CONSTANTS:
    "! 0 — 체크 통과(또는 check indicator가 no check, W-08).
    granted        TYPE i VALUE 0,
    "! 4 — authorization은 있으나 값 불일치, 또는 잘못된 필드/필드 수 초과(W-03 td4).
    value_mismatch TYPE i VALUE 4,
    "! 12 — 해당 object에 대한 authorization 자체가 없음(W-03 td12).
    no_auth        TYPE i VALUE 12,
    "! 40 — FOR USER 사용자명이 유효하지 않음(W-03 td40, W-07).
    invalid_user   TYPE i VALUE 40.
ENDINTERFACE.


"! ACTVT 활동 코드 상수(W-10·L-05). CRUD 표준 매핑.
INTERFACE lif_actvt.
  CONSTANTS:
    "! 01 — create(생성).
...
"! 실 AUTHORITY-CHECK 구문을 감싸는 production 구현(L-01). 현재 로그온 사용자에 의존하므로
...
CLASS lcl_real_authority IMPLEMENTATION.
  METHOD lif_authority~check.
    ...
    " 실무에서는 object·field를 동적으로 다룰 수 없으므로(리터럴 권장, W-02), 케이스별로 쓴다.
    ...
    CASE object.
      WHEN 'S_TCODE'.
        ...
        result = sy-subrc.
      WHEN 'S_CARRID'.
        " CARRID(데이터)와 ACTVT(활동)를 각각 지정 — W-03b canonical 예제.
    ...
    ENDCASE.
    " user(FOR USER, W-07)는 데모 production 경로에서 다루지 않는다 — 외부 유저명 지정은 보안 위험.
  ENDMETHOD.
ENDCLASS.
...
"! 통과 조건(W-01b): 같은 object의 authorization 인스턴스 중 적어도 하나가, 지정된 모든
...
CLASS lcl_authority_buffer DEFINITION CREATE PUBLIC.
  PUBLIC SECTION.
    ...
    "! @parameter instance  | authorization 인스턴스 번호(같은 번호 = 같은 인스턴스, W-01b)
    ...
    "! 유효 사용자 등록(FOR USER 데모). 미등록 사용자를 FOR USER로 지정하면 subrc=40(W-07).
...
CLASS lcl_authority_buffer IMPLEMENTATION.
  ...
  METHOD lif_authority~check.
    ...
    " FOR USER 지정 사용자가 유효하지 않으면 subrc=40(W-07). 현재 사용자(user 비움)는 항상 유효.
    IF user IS NOT INITIAL AND NOT line_exists( known_users[ table_line = user ] ).
      result = lif_subrc=>invalid_user.
      RETURN.
    ENDIF.

    " object에 대한 authorization 인스턴스가 하나도 없으면 subrc=12(W-03 td12).
    ...
    " OR across authorizations: 적어도 한 인스턴스가 모든 field를 커버하면 통과(W-01b).
    ...
    " 인스턴스는 있으나 어느 것도 모든 field 검사값을 포함하지 못함 -> subrc=4(W-03 td4).
  ...
  METHOD instance_covers_all.
    " AND across fields: 지정된 모든 field 각각이 통과해야 한다. DUMMY/빈 값은 체크 생략(W-05).
    LOOP AT fields INTO DATA(field).
      IF field-dummy = abap_true.
        " DUMMY field는 value set 무관하게 통과로 간주(W-05).
        CONTINUE.
      ENDIF.
