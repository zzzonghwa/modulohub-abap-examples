...
CLASS zcl_modulo_txn03_auth DEFINITION
  ...
  PUBLIC SECTION.
    ...
    "! 권한 체커 의존성을 주입한다(L-01). 비우면 실 AUTHORITY-CHECK를 감싼 production 구현 사용.
    ...
    "! S_CARRID display 체크 — W-03b canonical 예제. CARRID(데이터)+ACTVT(활동) 동시 지정.
    ...
    "! sy-subrc 의미 구분(W-03b): 0/4/12를 사람이 읽는 라벨로 분류한다.
    ...
    "! DUMMY 패턴(W-05·L-06): 데이터 field는 체크하지 않고 ACTVT(활동)만 체크한다.
    ...
    "! FOR USER(W-07): 다른 사용자의 권한을 체크한다. 외부 입력 유저명은 보안 위험 —
  ...
  PRIVATE SECTION.
    ...
    "! sy-subrc 코드를 사람이 읽는 라벨로 변환(W-03 의미 구분).
    METHODS subrc_label
...
CLASS zcl_modulo_txn03_auth IMPLEMENTATION.
  METHOD constructor.
    " 의존성 주입(L-01): 미지정 시 실 AUTHORITY-CHECK를 감싼 production 구현을 쓴다.
  ...
  METHOD if_oo_adt_classrun~main.
    ...
    " 한 authorization 인스턴스에 CARRID={AA,LH}·ACTVT={03} 부여(W-01b: 같은 인스턴스 = AND 만족).
    ...
    out->write( `결과는 OR(인스턴스) · AND(필드) 논리(W-01b)와 sy-subrc 체계(W-03)를 따른다.` ).
  ENDMETHOD.
  ...
  METHOD can_display_carrier.
    " W-03b 패턴: CARRID(데이터)+ACTVT='03'(display) 동시 지정 — 둘 다 충족해야 통과(AND).
    DATA(subrc) = authority->check(
      object = 'S_CARRID'
  ...
  METHOD can_do_activity.
    " DUMMY(W-05): CARRID는 dummy=abap_true로 체크 생략 -> ACTVT만 본다(global authorization).
    DATA(subrc) = authority->check(
      object = 'S_CARRID'
  ...
  METHOD check_for_user.
    " FOR USER(W-07): 지정 사용자의 권한을 체크. 무효 사용자명은 sy-subrc = 40.
    DATA(subrc) = authority->check(
      object = 'S_CARRID'
      fields = VALUE #( ( name = `CARRID` value = CONV string( carrier ) )
                        ( name = `ACTVT`  value = lif_actvt=>display ) )
