...
"! <p>성능 — 측정 도구와 알고리즘 복잡도(노트 07-5). 측정 도구는 대화형 tcode이므로</p>
"! <p>이 클래스는 그 도구들이 *진단하는* 패턴을 실행 가능한 형태로 자체완결 시연한다.</p>
"! <p>모든 짝(나쁨 vs 좋음)은 결과가 같고 비용만 다르다 — "측정 후 최적화" 원칙.</p>
"! <p>측정 도구: ST05(SQL Trace, DB 호출)·SAT(Runtime Analysis, ABAP 핫스팟)·SQLM(운영 SQL 모니터).</p>
"! <p>ATC 성능 룰: LOOP 내 SELECT·중첩 LOOP·표준 테이블 선형 검색·SELECT *를 정적으로 잡는다.</p>
"! <ul>
"! <li>알고리즘 복잡도: match_nested(O(n*m)) vs match_hashed/match_sorted(O(n)).</li>
"! <li>보조 키: secondary_key_count(since 7.02). 필드 심볼: assign_count vs into_count.</li>
"! <li>존재 확인: exists_no_fields(TRANSPORTING NO FIELDS) vs exists_select(SELECT @abap_true).</li>
"! <li>블록 처리: block_merge vs row_by_row. 메모리: clear_then_size vs free_then_size.</li>
"! <li>code pushdown: sum_pushdown(SELECT SUM) vs sum_in_abap(REDUCE). WHERE: and_count vs or_count.</li>
"! </ul>
