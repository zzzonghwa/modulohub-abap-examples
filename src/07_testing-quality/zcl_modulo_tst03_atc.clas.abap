...
"! <p>ATC(ABAP Test Cockpit)·정적 품질 — 구문·패턴을 자체완결로 시연한다.</p>
...
"! <ul>
"! <li>priority 1/2/3 분류: finding 우선순위 -> 운반 게이트 동작.</li>
"! <li>missing-WHERE: WHERE 있는 SELECT가 prio2를 피한다.</li>
"! <li>ORDER BY 없는 INDEX 읽기: 정렬 후 첫 행 접근이 prio3을 피한다.</li>
"! <li>##NEEDED·##NO_HANDLER·##NO_TEXT/"#EC 억제.</li>
"! <li>baseline exempt vs suppress·exemption 반패턴·억제엔 이유주석.</li>
"! <li>커스텀 체크 토큰 스캔·INFORM kind note/error·카테고리 position.</li>
"! </ul>
CLASS zcl_modulo_tst03_atc DEFINITION
  ...
  PUBLIC SECTION.
    ...
    "! 심각도 코드 -> 라벨. 중첩 IF 대신 SWITCH로 분기(ATC "복잡도" finding 회피).
    ...
    "! priority -> 운반(transport) 게이트 동작. 1·2는 차단, 3은 알림만.
    ...
    "! missing-WHERE 회피 — WHERE 있는 SELECT는 prio2 finding을 만들지 않는다.
    ...
    "! baseline 추가 모드 — exempt(승인 워크플로) vs suppress(단순 억제).
    ...
    "! exemption 신청 반패턴 — "복잡/단순해서 면제"는 거부한다.
    ...
    "! 억제 메타데이터 — pragma/의사주석 억제엔 같은 줄 이유 주석이 필수.
    ...
    "! 커스텀 체크 토큰 스캔 — 한 statement의 token 중 특정 키워드 출현 횟수.
    ...
    "! INFORM kind — finding을 note(정보) 또는 error(적색)로 분류.
    ...
    "! 커스텀 카테고리 목록 위치 — position '001'은 맨 위, '999'는 맨 아래.
...
CLASS zcl_modulo_tst03_atc IMPLEMENTATION.
  ...
  METHOD transport_gate.
    " prio1·2는 운반 릴리스를 차단(BLOCK), prio3은 알림(NOTIFY)만.
  ...
  METHOD count_min_seats.
    " WHERE를 붙여 missing-WHERE(prio2) finding을 만들지 않는다.
  ...
  METHOD top_connid_by_seats.
    " ORDER BY를 명시해 "ORDER BY 없는 INDEX 1 읽기"(prio3) finding을 피한다.
  ...
  METHOD count_tokens.
    " RUN 메서드의 ref_scan->tokens 구조를 단순화: 구문을 token으로 분해해 키워드를 센다.
  ...
  METHOD inform_kind.
    " INFORM의 p_kind = c_error(적색) vs c_note(정보 열).
    result = COND #( WHEN is_error = abap_true THEN `ERROR` ELSE `NOTE` ).
  ENDMETHOD.

  METHOD category_rank.
    " position '001' 맨 위(Code Pal), '999' 맨 아래(ABAP Open Checks).
