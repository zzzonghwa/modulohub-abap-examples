...
"! 노트 H절(주장18 spy 패턴)의 수동 등가물: "어떤 메서드가 몇 번·무엇으로 호출됐나"를 본다.
...
"! 부분 구현 더블 — INTERFACES ... PARTIALLY IMPLEMENTED(주장14)는 테스트 클래스에서만 허용.
...
"! 커스텀 단언 헬퍼(주장39·주장32) — 도메인을 이해하는 단언을 표준과 같은 계약(msg)으로 제공.
...
"! 시간대별 인사 — 클래스명은 공통 when(by_time)을, 메서드명은 then을 담는다(주장34).
CLASS ltcl_greeting_by_time DEFINITION FINAL FOR TESTING
  ...
  PRIVATE SECTION.
    ...
    "! given 추출(주장27): 고정 시각으로 인사기를 직접 조립해 돌려준다.
    METHODS greeter_at
...
CLASS ltcl_greeting_by_time IMPLEMENTATION.
  METHOD setup.
    " 픽스처(주장6·9): 매 테스트 전 cut를 새로 만든다. teardown은 불필요(런타임이 새 인스턴스 생성).
    cut = NEW #( ).
  ENDMETHOD.
  ...
  METHOD boundary_at_noon.
    " 경계값(주장13 내용 단언): 12시는 정확히 오후로 넘어가는 첫 시각이다.
  ...
  METHOD greeting_is_in_lexicon.
    " 커스텀 단언 헬퍼(주장39): 정확한 문자열이 아니라 "유효한 인사말" 속성을 단언(주장13-품질).
    lcl_greeting_assert=>assert_is_valid_greeting( cut->greeting_at( 23 ) ).
  ENDMETHOD.
ENDCLASS.


"! 업무시간 판정 — bool·initial·char_cp·that 등 단언 다양체(E절)를 모은다.
...
CLASS ltcl_business_hours IMPLEMENTATION.
  ...
  METHOD differs_across_times.
    " assert_differs(주장11): 두 기본형이 서로 다름을 단언. 아침≠저녁 인사여야 한다.
    cl_abap_unit_assert=>assert_differs( act = cut->greeting_at( 8 ) exp = cut->greeting_at( 20 ) ).
  ENDMETHOD.
ENDCLASS.


"! 발송 분기 — 더블 주입으로 lcl_dispatcher를 격리하고 상호작용·예외를 검증한다(H·F절).
"! class_setup/class_teardown(주장6·7)도 시연: 클래스 전체 1회 공유 준비.
...
CLASS ltcl_dispatch IMPLEMENTATION.
  METHOD class_setup.
    " class_setup(주장7): 전체 클래스 시작 전 1회. 변경 없는 공유 상수만 준비한다.
    office_hour = 10.
  ENDMETHOD.

  METHOD class_teardown.
    " class_teardown(주장9): 로컬 더블만 쓰므로 외부 정리 대상 없음 — 형태 시연용.
    RETURN.
  ENDMETHOD.
  ...
  METHOD within_limit_sends.
    " RAISING 전달 패턴(주장38): happy path는 예외가 무의미하므로 메서드 시그니처에
  ...
  METHOD over_limit_raises.
    " 예외 기대 패턴(주장37): TRY-호출-FAIL-CATCH. 예외가 안 나면 FAIL이 실패시킨다.
    TRY.
