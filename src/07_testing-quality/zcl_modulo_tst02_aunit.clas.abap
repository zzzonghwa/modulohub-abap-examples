...
"! <p>ABAP Unit 심화 — 의존을 인터페이스로 분리(lif_clock·lif_notifier)하고</p>
"! <p>생성자 주입하면, 테스트가 시간·발송 같은 비결정/부수효과 의존을 더블로 대체해</p>
"! <p>결정적으로 검증할 수 있다. 이 글로벌 클래스는 더블을 주입한 도메인 객체의 얇은</p>
"! <p>파사드이고, 테스트 패턴의 실물은 테스트 인클루드에 모여 있다:</p>
"! <ul>
"! <li>픽스처(setup·class_setup): 매 테스트 전 cut 재생성·1회성 공유 준비.</li>
"! <li>단언 다양체: assert_equals·true·false·differs·bound·initial·char_cp.</li>
"! <li>예외 기대 패턴: TRY-CALL-FAIL-CATCH, RAISING 전달.</li>
"! <li>수동 더블: 스텁(lcl_fixed_clock)·스파이(lcl_clock_spy)·PARTIALLY IMPLEMENTED.</li>
"! <li>커스텀 단언 헬퍼 클래스, Only Mock What's Needed 실 구현 활용.</li>
"! <li>given-when-then 서브메서드 추출, 픽스처 4종.</li>
"! </ul>
...
CLASS zcl_modulo_tst02_aunit IMPLEMENTATION.
  ...
  METHOD dispatch_within_limit_at.
    " 파사드: 도메인 예외를 흡수해 음수 코드로 변환(전파 vs 흡수 대비).
    TRY.
